...
1. 确认当前产品版本为 `v1.0.4`（commit `release: v1.0.4` / tag `v1.0.4`），目录版本为 `20260823-011b26367022`。
2. 阅读本文、`docs/RELEASE_PROCESS.md`、`README.md`、`CHANGELOG.md` 和 `VERSION.json`。
3. 执行 `git log --oneline --decorate -10` 和 `git tag -n`，确认 `v1.0.4` tag 与 release commit。
...
| --- | --- |
| 产品名 | 剑网3掉落工坊 |
| 最新已发布版本 | `v1.0.4`（tag `v1.0.4`） |
| 当前工作区版本 | `v1.0.4`（正式稳定版） |
...
| 2026-08-25 23:31 | `ac4c851` | `release: v1.0.3`，将默认插件数据注册至用户自定义物品库 |
| 2026-08-27 14:00 | `release: v1.0.4` | `release: v1.0.4`，基线质量加固、业务语义理顺与数据安全防线修复 |
...
1. **当前工作区状态**：已完成基线加固与多轮深度复核，已正式发布为 `v1.0.4`（commit `release: v1.0.4` / tag `v1.0.4`）。
2. **版本元数据全链路对齐**：`package.json`、`package-lock.json`、`VERSION.json`、`constants.ts`、`README.md` 与 `CHANGELOG.md` 均严格对齐为 `1.0.4`（稳定版）。
3. **更新日志与历史完整**：`CHANGELOG.md` 完整记录了 1.0.4、1.0.3、1.0.2、1.0.1、1.0.0 的演进历史，不存在版本断层。
4. **门禁与测试完备**：`npm run check` 退出码 0，5 个测试文件、140 项测试全部通过（含 40 项全量参数化移除测试、8 项官方掉落动态分类测试、文件大小前置拦截测试与深度往返断言）。
5. **已知设计权衡（后续迭代）**：
   - 导出仍包含约 23 项拾取插件附加默认设置（P1-01），后续可评估增加“精简纯净导出”选项；
